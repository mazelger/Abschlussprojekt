...
def spiel_starten(daten, highscores):
    ...
    while True:
        ...
        auswahl = input("Welches ist besser? (1 oder 2, q zum Beenden): ")

        if auswahl == "q":
            break

        if eigenschaft == 'Beschleunigung(m/s)':  # Kleinere Beschleunigung ist besser
            korrekt = (wert1 <= wert2 and auswahl == "1") or (wert2 <= wert1 and auswahl == "2")
        else:  # Bei allen anderen ist größer besser
            korrekt = (wert1 >= wert2 and auswahl == "1") or (wert2 >= wert1 and auswahl == "2")
